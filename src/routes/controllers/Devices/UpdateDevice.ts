...
export async function updateDevice(
  ...
  if (!Value.Check(UpdateDeviceRequest, data)) {
    if (data.length > 0)
      res.status(400).send(new ErrorResponse(400, 'You need to edit at least 1 property of the device!'));
    else
      res
        .status(400)
        .send(
  ...
  //-> Path Collisions
    const pathCollisions = await Devices.findAll({
        where: {device: `${data.ipAddress ?? oldProps.ipAddress}|${data.rtspPort ?? oldProps.rtspPort}|${data.rtspPath ?? oldProps.rtspPath}`},
      });
  ...
  Devices.update({
    ...
    audio_disabled: 'audio_enabled' in data ? (data.audio_enabled ? 0 : 1) : originalDevice.dataValues.audio_enabled,
    substream_mode: data.substreamMode ?? originalDevice.dataValues.substreamMode,
    ...
    rtsp_rtp_prefer_tcp: data.preferTcp ?? originalDevice.dataValues.preferTcp,
    onvif_port: data.onvifPort ?? originalDevice.dataValues.onvifPort,
    hls_window_size: data.hls_window_size ?? originalDevice.dataValues.hls_window_size,
    hls_segment_duration: data.hls_segment_duration ?? originalDevice.dataValues.hls_segment_duration,
    hls_segment_size: data.hls_segment_size ?? originalDevice.dataValues.hls_segment_size,
    ...
    buffer_postrecording: data.buffer_postrecording ?? originalDevice.dataValues.buffer_postrecording,
    ...
    frame_downscale_factor: data.frame_downscale_factor ?? originalDevice.dataValues.frame_downscale_factor,
    invert: data.invert ?? originalDevice.dataValues.invert,
    max_motion_area: data.max_motion_area ?? originalDevice.dataValues.max_motion_area, 
    max_motion_frames: data.max_motion_frames ?? originalDevice.dataValues.max_motion_frames,
    min_motion_frames: data.min_motion_frames ?? originalDevice.dataValues.min_motion_frames,
    motion_algorithm: data.motion_algorithm ?? originalDevice.dataValues.motion_algorithm,
    motion_analysis_percentage: data.motion_analysis_percentage ?? originalDevice.dataValues.motion_analysis_percentage,
    motion_analysis_ssw_length: data.motion_analysis_ssw_length ?? originalDevice.dataValues.motion_analysis_ssw_length,
    motion_blend_ratio: data.motion_blend_ratio ?? originalDevice.dataValues.motion_blend_ratio,
    ...
    onvif_events_enabled: data.onvif_events_enabled ?? originalDevice.dataValues.onvif_events_enabled,
    ...
    schedule_override_global: data.schedule_override_global ?? originalDevice.dataValues.schedule_override_global,
    signal_type: data.signal_type ?? originalDevice.dataValues.signal_type,
    video_interval: data.video_interval ?? originalDevice.dataValues.video_interval,
    video_quality: data.video_quality ?? originalDevice.dataValues.video_quality,
  },{ where: { id: deviceId } }).then(async ()=> {
    res.status(200).send(
