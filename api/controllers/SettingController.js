...
const SettingController = () => {
  ...
  const getAllSettings = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        settings, getVaapiOptions, validSecureOptions, getLiveViewVideoOptions,
      });
    } catch (err) {
  ...
  const saveAllSettings = async (req, res) => {
    try {
      ...
      return res.status(200).json({ msg: 'Setting has been updated successfully', fieldsNotUpdated, fieldsUpdatedStatus });
    } catch (err) {
      return res.status(500).json({ msg: 'Internal server error', err });
    }
  };
